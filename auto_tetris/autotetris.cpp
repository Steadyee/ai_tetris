...
#define SCORE_THRESHOLD_GENERAL 1000
#define SCORE_THRESHOLD_DIFFICULT 2000
#define LEVEL_UP_INTERVAL 250
...
// 优化：将笔和刷子作为全局变量，只创建一次，减少每次绘制时的资源创建与销毁
static HPEN hPenNull = (HPEN)GetStockObject(NULL_PEN);
static HBRUSH hBrushGray = (HBRUSH)GetStockObject(GRAY_BRUSH);
...
// 优化：将画刷选择提取到循环外部，避免重复调用 SelectObject
VOID DrawTetris(HDC hdc)
{
	...
	for (int i = 0; i < PAINT_YNODE; i++)
	{
		for (int j = 0; j < GAME_XNODE; j++)
		{
			if (btCurGrid[i][j] > 0)
			{
				hBrush = getHbrush(btCurGrid[i][j]);
	...
	// 绘制正在下落图形
	hBrush = getHbrush(curTetris.color);
	SelectObject(hdc, hBrush);
	for (int i = 0; i < 4; i++)
	{
	...
	hBrush = getHbrush(nextTetris.color);
	SelectObject(hdc, hBrush);
	for (int i = 0; i < 4; i++)
	{
...
{
	if (iCurDif < GENERAL && iCurScore >= SCORE_THRESHOLD_GENERAL) {
		iCurDif++;
		iInterval -= LEVEL_UP_INTERVAL;
	}
	else if (iCurDif < DIFFICULT && iCurScore >= SCORE_THRESHOLD_DIFFICULT) {
		iCurDif++;
		iInterval -= LEVEL_UP_INTERVAL;
	}
}// 下落完成
...
{
	for (int i = 0; i < PAINT_YNODE; i++) {
		bool fullLine = std::all_of(btCurGrid[i], btCurGrid[i] + GAME_XNODE, [](int val) { return val > 0; });
		if (fullLine) {
			// 处理消除方块
		}
	}
